...
function mapProduct(data: any): Product {
  ...
  // Phân loại category theo 4 danh mục (mirror SQL example)
  let mappedCategory = "pastry";
  if (categoryName.includes("bánh kem") || categoryName.includes("bánh sinh nhật") || categoryName.includes("birthday")) {
    mappedCategory = "birthday";
  } else if (categoryName.includes("bánh mì") || categoryName.includes("bread")) {
    mappedCategory = "bread";
  } else if (categoryName.includes("cupcake")) {
    mappedCategory = "cupcake";
  } else if (categoryName.includes("bánh quy") || categoryName.includes("cookie")) {
    mappedCategory = "cookie";
  } else if (categoryName.includes("bánh ngọt") || categoryName.includes("pastry")) {
    mappedCategory = "pastry";
  }
...
export async function createProduct(product: { categoryId?: number; productName: string; flavor?: string; description?: string; price: number; imageUrl?: string }): Promise<any> {
...
export async function getCustomer(customerId: number): Promise<any> {
  return await request<any>(`/api/customers/${customerId}`);
}

export async function updateCustomer(customerId: number, payload: { fullName: string; phone?: string; address?: string; gender?: string | null; }): Promise<any> {
  return await request<any>(`/api/customers/${customerId}`, {
    method: "PUT",
