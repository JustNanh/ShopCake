...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setForm((old) => ({
          ...old,
          name: customer.fullName || old.name,
          phone: customer.phone || old.phone,
          address: customer.address || old.address,
        }));
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Cập nhật lại thông tin khách hàng (lưu form thông tin giao hàng) để lần sau tự điền
      try {
