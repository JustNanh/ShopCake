...
const shippingFeeForMethod = (method: string | null | undefined) => {
  if (!method) return 0;
  if (method.toLowerCase().includes("express")) return 1000; // VD: 10k
  if (method.toLowerCase().includes("standard")) return 2000; // VD: 20k
  // Default trong trường hợp chưa định nghĩa
  return 2000;
};
    ...
    <div className="container py-10">
                  ...
                  <div key={order.orderId} className="rounded-lg border p-4">
                    <div className="flex justify-between items-start flex-col md:flex-row gap-3">
                      <div>
                        ...
                        <div className="text-sm">Tiền hàng: {formatPrice(Number(order.totalAmount))}</div>
                        <div className="text-sm">Phí giao hàng: {formatPrice(shippingFeeForMethod(order.shippingMethod))}</div>
                        <div className="text-sm font-bold">Tổng (cả phí): {formatPrice(Number(order.totalAmount) + shippingFeeForMethod(order.shippingMethod))}</div>
                      </div>
      ...
      {payingOrder && (
        <PaymentModal
          ...
          totalAmount={Number(payingOrder.totalAmount) + shippingFeeForMethod(payingOrder.shippingMethod)}
