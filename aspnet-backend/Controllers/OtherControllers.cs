...
public class CategoriesController : ControllerBase
{
    ...
    /// <summary>Lấy tất cả danh mục (Public)</summary>
    [HttpGet]
    public async Task<IActionResult> GetAll() => Ok(await _db.Categories.ToListAsync());

    /// <summary>Thêm danh mục mới (Chỉ Admin)</summary>
    ...
    /// <summary>Xóa danh mục (Chỉ Admin)</summary>
...
public class CustomersController : ControllerBase
{
    ...
    /// <summary>Lấy danh sách khách hàng (Chỉ Admin)</summary>
    [HttpGet]
    [Authorize(Roles = "Admin")]
    public async Task<IActionResult> GetAll() =>
        Ok(await _db.Customers.Select(c => new { c.CustomerId, c.FullName, c.Phone, c.Email, c.Address, c.Gender }).ToListAsync());

    /// <summary>Lấy chi tiết 1 khách hàng (User đang đăng nhập hoặc Admin)</summary>
    [HttpGet("{id}")]
    public async Task<IActionResult> GetById(int id)
    {
        // Ghi chú: Có thể thêm logic kiểm tra id có khớp với User.FindFirst("customer_id") không nếu muốn bảo mật hơn cho User thường
    ...
    /// <summary>Xóa khách hàng (Chỉ Admin)</summary>
    [HttpDelete("{id}")]
    [Authorize(Roles = "Admin")]
    public async Task<IActionResult> Delete(int id)
    {
...
public class ReviewsController : ControllerBase
{
    ...
    /// <summary>Lấy đánh giá của 1 sản phẩm (Public)</summary>
